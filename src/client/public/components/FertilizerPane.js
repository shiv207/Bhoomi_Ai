...
class FertilizerPane {
  ...
  init() {
    ...
    console.log('🌾 FertilizerPane initialized');
  }
    ...
    container.innerHTML = `
      ...
      <div class="fertilizer-header">
        ...
        <div class="fertilizer-controls">
          <button class="fertilizer-btn" onclick="window.fertilizerPane.copyChecklist()">Copy Checklist</button>
          <button class="fertilizer-btn" onclick="window.fertilizerPane.showSources()">Show Sources</button>
          <button class="fertilizer-btn" onclick="window.fertilizerPane.hide()">✕</button>
        </div>
      </div>
   ...
   */
  async fetchAndDisplay(location, crop, soil, expectedYield) {
    this.show();
    ...
    try {
      ...
      content.innerHTML = `
        <div class="fertilizer-error">
          ...
          ${error.message}<br>
          <button class="fertilizer-btn" onclick="window.fertilizerPane.retry()" style="margin-top: 8px;">Retry</button>
        </div>
      `;
    }
  }
  ...
  renderContent(data) {
    ...
    content.innerHTML = `
      ...
      <button class="fertilizer-copy-btn" onclick="window.fertilizerPane.copyChecklist()">
        📋 Copy as Checklist
      </button>
    `;
  }
  ...
  copyChecklist() {
    ...
    navigator.clipboard.writeText(checklist).then(() => {
      ...
      const btn = document.querySelector('.fertilizer-copy-btn');
      if (btn) {
  ...
  retry() {
    ...
    // Could store last parameters and retry
  }

  /**
   * Auto-trigger from crop recommendations
   */
  autoTrigger(fertilizerContext) {
    if (!fertilizerContext) return;
    
    const { location, detectedCrops, soilType, expectedYield } = fertilizerContext;
    
    if (detectedCrops && detectedCrops.length > 0) {
      console.log('🌾 Auto-triggering fertilizer pane for detected crops:', detectedCrops);
      this.fetchAndDisplay(
        location || 'India',
        detectedCrops[0] || 'general',
        soilType || 'mixed',
...
console.log('🌾 FertilizerPane script loaded');
